...
export class AdminLaboratoriosComponent implements OnInit, OnDestroy {
  ...
  filter(val: string): string[] {
    return this.facultades.filter(
      (option) =>
        option.nombre.trim().toLowerCase().indexOf(val.trim().toLowerCase()) !==
        -1
    );
  }
  ...
  estructurarLab(key) {
    ...
    const promise = new Promise((resolve, reject) => {
      this.servicioMod2.buscarLab(key).then((labo) => {
        ...
        this.servicioMod2
          .buscarSede(
            laboratorio.headquarter ? laboratorio.headquarter : "tygd"
          )
          .then((se) => {
            const sede = se.data();
            this.servicioMod2
              .buscarSubSede(laboratorio.subHq ? laboratorio.subHq : "mfij")
              .then((sub) => {
                const subsede = sub.data();
                this.servicioMod2
                  .buscarDirector(laboratorio.facilityAdmin)
                  .then((dueno) => {
                    ...
                    this.labestructurado = {
                      ...
                      sede: {
                        id: laboratorio.headquarter,
                        nombre: sede ? sede.cfName : "",
                      },
                      subsede: {
                        id: laboratorio.subHq,
                        nombre: subsede
                          ? subsede.cfAddrline2 + " - " + subsede.cfAddrline1
                          : "",
                      },
                      ...
                      equipos: this.estructurarEquipos(
                        laboratorio.relatedEquipments
                      ),
                      personal: this.estructurarPersonas(
                        laboratorio.relatedPers
                      ),
                      facultades: this.estructurarFacultades(
                        laboratorio.faculties
                      ),
                      departamentos: this.estructurarDepartamentos(
                        laboratorio.departments
                      ),
                      espacios: this.estructurarSpace(
                        laboratorio.relatedSpaces,
                        laboratorio.mainSpace
                      ),
                      cambios: laboratorio.suggestedChanges,
                      disponibilidad:
                        typeof laboratorio.cfAvailability == "string"
                          ? []
                          : laboratorio.cfAvailability,
                      condiciones: laboratorio.cfConditions,
                      estado: estadoLab,
                    };
                    if (duenoLab) {
                      this.labestructurado.director =
                        duenoLab.cfFirstNames + " " + duenoLab.cfFamilyNames;
                    } else {
                      this.labestructurado.director = `Sin asignar`;
                    }
                    // if (laboratorio.facilActivity.extension) {
                    const servicesol = this.estructurarServicios(
                      laboratorio.relatedServices
                    );
                    ...
                    if (laboratorio.facilActivity.research) {
                      this.labestructurado[
                        "proyectos"
                      ] = this.estructurarProyectos(
                        laboratorio.relatedProjects
                      );
                    }
                    if (laboratorio.facilActivity.teaching) {
                      this.labestructurado[
                        "practicas"
                      ] = this.estructurarPracticas(
                        laboratorio.relatedPractices
                      );
                    }
                    this.cambios = this.pendientes(
                      laboratorio.suggestedChanges
                    );
                    if (this.labestructurado) {
                      resolve();
                    } else {
      ...
      if (item.hasOwnProperty(clave)) {
        if (item[clave]) {
          this.servicioMod2.buscarServicio(clave).then((data) => {
            ...
            arr.push(serv);
            this.servicioMod2
              .getSolicitudesServiciosForId(clave)
              .then((dataSol) => {
                dataSol.forEach((doc) => {
      ...
      if (item.hasOwnProperty(clave)) {
        if (item[clave]) {
          this.servicioMod2.buscarPersona(clave).then((data) => {
            ...
            let persona = {};
            this.servicioMod2
              .buscarUsuario(pers.user ? pers.user : "123")
              .then((dataper) => {
                // funciona con una programacion, cuando hayan mas toca crear otro metodo
                if (dataper.data()) {
                  persona = {
    ...
    this.servicioMod2.getSubSedes().then((data) => {
      data.forEach((doc) => {
        this.subsedesStatic.push({
          id: doc.id,
          nombre: doc.data().cfAddrline2 + " - " + doc.data().cfAddrline1,
    ...
    this.servicioMod2.getDepartamentos(idfacul).then((data) => {
      data.forEach((doc) => {
        this.departamentos.push({
          id: doc.id,
          idfacul: idfacul,
          nombre: doc.data().departmentName,
        });
        this.departamentosStatic.push({
          id: doc.id,
          idfacul: idfacul,
          nombre: doc.data().departmentName,
        });
  ...
  getRoles(rol) {
    ...
    console.log("Nivel 2", this.moduloNivel2, "Nivel 3 ", this.moduloNivel3);
    ...
    if (this.moduloNivel2 || this.moduloNivel3 || this.moduloNivel25) {
      swal({
        ...
        if (result.value) {
          ...
          this.infolab.faculties = this.estructurarEnvioSugerenciaFacDep(
            this.listaFacultades,
            "faculties"
          );
          this.infolab.departments = this.estructurarEnvioSugerenciaFacDep(
          ...
          this.servicioMod2
            .Trazability(
              ...
              "update",
              "cfFacil",
              this.labestructurado.uid,
              this.infolab
            )
            .then(() => {
              this.servicioMod2
                .updateDocLaboratorio(this.labestructurado.uid, this.infolab)
                .then((data) => {
                  console.log(1011, this.infolab);
                  const labEmail = this.infolab.otros.email;
                  const updatedAt = this.infolab.updatedAt;
                  const active = this.infolab.active;
                  const laboratorio = { labEmail, updatedAt, active };
                  console.log(935, laboratorio);
                  this.servicioMod2.updateCacheLaboratorios(
                    this.labestructurado.uid,
                    laboratorio
                  );
                  swal.close();
      ...
      if (dataEstructurada.length !== 0) {
        swal({
          ...
          if (result.value) {
            ...
            this.servicioMod2
              .buscarPersona(this.servicioMod2.getLocalStoragePers().cfPers)
              .then((person) => {
                aux.suggestedChanges.push({
                  ...
                  nombre:
                    person.data().cfFirstNames +
                    " " +
                    person.data().cfFamilyNames,
                  estado: "pendiente",
                });
                ...
                this.servicioMod2
                  ...
                  .then(() => {
                    this.servicioMod2
                      .setDocLaboratorio(this.labestructurado.uid, aux)
                      .then(() => {
                        swal.close();
                        swal({
                          type: "success",
  ...
  enviarSugerencia() {
    swal({
      type: "warning",
      title:
        "¿Está seguro que desea editar los datos actuales con la sugerencia seleccionada?",
      showCancelButton: true,
      confirmButtonText: "Sí, Solicitar",
      cancelButtonText: "No, Cancelar",
    }).then((result) => {
      if (result.value) {
              ...
              if (aux.length !== 2) {
                if (aux === "faculties" || aux === "departments") {
                  if (this.sugerencia.data[cont].quitar) {
                    const obj = {};
                    obj[aux] = this.estructurarEnvioSugerenciaFacDep(
                      this.sugerencia.data[cont].info,
                      aux
                    );
                    cambio[aux[0]] = {};
                    this.servicioMod2
                      ...
                      .then(() => {
                        this.servicioMod2.updateDocLaboratorio(
                          this.labestructurado.uid,
                          obj
                        );
                      });
                  } else {
                    cambio[aux[0]] = this.estructurarEnvioSugerenciaFacDep(
                      this.sugerencia.data[cont].info,
                      aux
                    );
                  }
                  ...
                  cambio[
                    aux[0]
                  ] = this.estructurarEnvioSuferenciaDisponibilidad(
                    this.sugerencia.data[cont].info,
                    this.sugerencia.data[cont].quitar
                  );
                } else if (aux === "cfEAddr") {
                  if (this.sugerencia.data[cont].quitar) {
                    this.EnviarcfEAddr(
                      this.sugerencia.data[cont].infoaux,
                      false
                    );
                  } else {
                  ...
                  if (this.sugerencia.data[cont].quitar) {
                    const obj = {};
                    obj[aux] = this.estructurarEnvioSugerenciaActividad(
                      this.sugerencia.data[cont].infoaux,
                      false
                    );
                    this.servicioMod2
                      ...
                      .then(() => {
                        this.servicioMod2.setDocLaboratorio(
                          this.labestructurado.uid,
                          obj
                        );
                      });
                    ...
                    cambio[aux[0]] = {};
                    cambio[aux[0]] = this.estructurarEnvioSugerenciaActividad(
                      this.sugerencia.data[cont].info,
                      true
                    );
                  }
        ...
        cambio["suggestedChanges"] = this.cambiarEstadoSugerencia(
          this.sugerencia.pos,
          "aprobado"
        );
        this.servicioMod2
          ...
          .then(() => {
            this.servicioMod2
              .setDocLaboratorio(this.labestructurado.uid, cambio)
              .then((data) => {
                swal.close();
                swal("Cambios aprobados", "", "success");
                this.sugerencia = undefined;
                this.limpiarData();
                // tslint:disable-next-line:max-line-length
                this.obs.changeObject({
                  nombre:
                    this.labestructurado.nombre.nom1 +
                    this.labestructurado.nombre.nom2,
                  uid: this.labestructurado.uid,
                });
              });
          });
      } else if (result.dismiss === swal.DismissReason.cancel) {
  ...
  estructurarEnvioActividades() {
    const retirado = this.elementosRetirados(
      this.listaTelefonos,
      this.labestructurado.telefonos
    );
    this.EnviarcfEAddr(retirado, false);
    const agregado = this.elementosRetirados(
      this.labestructurado.telefonos,
      this.listaTelefonos
    );
  ...
  EnviarcfEAddr(data, accion) {
    for (let i = 0; i < data.length; i++) {
      ...
      if (accion) {
        ...
        this.servicioMod2
          .addAddrLaboratorio(this.labestructurado.uid, element.nombre)
          .then((data) => {
            data.get().then((doc) => {
        ...
        this.servicioMod2
          ...
          .then(() => {
            this.servicioMod2.deleteAddrLaboratorio(
              this.labestructurado.uid,
              element.id
            );
          });
  ...
  desaprobarSugerencia() {
    ...
    this.servicioMod2
      ...
      .then(() => {
        this.servicioMod2
          .updateDocLaboratorio(this.labestructurado.uid, cambio)
          .then((data) => {
            swal.close();
            swal({
              type: "success",
  ...
  estructurarDataCambios() {
    ...
    for (let i = 0; i < aux1.length; i++) {
      ...
      if (element.length !== 2) {
        if (
          aux1[i] === "facultades" ||
          aux1[i] === "departamentos" ||
          aux1[i] === "disponibilidad" ||
          aux1[i] === "telefonos"
        ) {
          const sugeridos = [];
          ...
          for (const key in this.infolab.facilActivity) {
            if (this.infolab.facilActivity.hasOwnProperty(key)) {
              if (
                this.infolab.facilActivity[key] !==
                this.labestructurado.objectActividad[key]
              ) {
                if (this.infolab.facilActivity[key]) {
                  sugeridosAct.push({
                    id: key,
                    nombre: this.listaActividad.find((o) => o.id === key).name,
                  });
                } else {
                  retiradosAct.push({
                    id: key,
          ...
          if (auxiliar !== this.infolab[element2]) {
            data.push({
              llave: aux2[i],
              nombre: nombre,
              infoaux: infoauxiliar,
              info: this.infolab[element2],
              cambio: false,
            });
          }
        }
        ...
        if (
          this.labestructurado[element[0]][element[1]] !==
          this.infolab[element2[0]][element2[1]]
        ) {
          data.push({
            llave: aux2[i],
            nombre: element2[1],
            infoaux: infoauxiliar,
            info: this.infolab[element2[0]][element2[1]],
            cambio: false,
          });
        }
      }
  ...
  cambiarEspacioSel(item) {
    if (item !== "inicial") {
      ...
      // this.espaciosel.map = "https://gdurl.com/Kyzs/viewer";
      if (this.espaciosel.map && this.isValidURL(this.espaciosel.map)) {
  ...
  limpiarData() {
    ...
    this.infolab.active =
      this.labestructurado.estado == "Activo" ? true : false;
    console.log(1668, this.listaDisponibilidad, this.labestructurado);
  ...
  agregarElemento(list) {
    if (
      (this.fac != undefined && this.fac.trim() != "" && list === "facultad") ||
      ...
      const objeto = {};
      if (list === "facultad") {
        this.selectfacul = this.facultades.find(
          (o) => o.nombre === this.fac
        ).id;
  ...
  agregarDisponibilidad() {
    if (this.selectdia !== 0) {
      const cadena =
        this.diassemana[this.selectdia - 1].nombre +
        " : " +
        this.selectHinicio +
        "-" +
        this.selectHFinal;
      ...
      if (!val) {
        ...
        swal({
          type: "error",
          title:
            "El dia seleccionado ya se encuentra agregado, por favor eliminelo de la disponibilidad para poderlo ingresar de nuevo",
          showConfirmButton: true,
        });
      }
    } else {
  ...
  agregarTelefonos() {
    if (this.telefono != undefined && this.telefono.trim() != "") {
      this.listaTelefonos.push({
        id: this.listaTelefonos.length,
        nombre: this.telefono,
      });
      if (this.moduloPermiso) {
        this.listaTelSugeridos.push({
          id: this.listaTelefonos.length,
          nombre: this.telefono,
        });
      }
