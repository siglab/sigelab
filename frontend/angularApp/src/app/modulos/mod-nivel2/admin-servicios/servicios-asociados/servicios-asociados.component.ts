...
export class ServiciosAsociadosComponent implements OnInit, OnDestroy {
      ...
      if (data.length !== 0) {
        ...
        this.servicioMod2.getCollectionServicios(data.uid).then((servicios) => {
          ...
          this.servicioMod2.buscarLab(this.lab_id).then((labo) => {
            this.equipos = this.estructurarEquipos(
              labo.data().relatedEquipments
            );
  ...
  cambiardata(item) {
    ...
    $("html, body").animate(
      { scrollTop: $("#serviceDetails").offset().top - 55 },
      800
    );
  }
  ...
  cambiarDatosEditar() {
    ...
    this.dataSourceEquipvin = new MatTableDataSource(
      this.itemsel.infoServ.equipos
    );
  }
  ...
  agregarServicio() {
    ...
    if (this.validarDatosSrv(this.srv)) {
      ...
      this.servicioMod2.addServicio(this.srv).then((data) => {
        ...
        this.servicioMod2
          .Trazability(this.user.uid, "create", "cfSrv", data.id, this.srv)
          .then(() => {
              ...
              .then(() => {
                ...
                if (this.variaciones.length !== 0) {
                  for (let i = 0; i < this.variaciones.length; i++) {
                    ...
                    this.servicioMod2
                      .addVariaciones(data.id, element)
                      .then((doc) => {
                        // swal.close();
                        this.servicioMod2
  ...
  editarServicio() {
    ...
    if (this.validarDatosSrv(this.srv)) {
      ...
      for (let i = 0; i < this.itemsel.infoServ.equipos.length; i++) {
        ...
        this.servicioMod2
          .Trazability(this.user.uid, "update", "cfEquip", equipo.id, srvequip)
          .then(() => {
      ...
      this.servicioMod2
        ...
        .then(() => {
          this.servicioMod2
            .updateServicio(this.itemsel.infoServ.uid, this.srv)
            .then(() => {
              for (let j = 0; j < this.variaciones.length; j++) {
                const variacion = this.variaciones[j];
                if (variacion.id === "nuevo") {
                  this.servicioMod2
                    .addVariaciones(this.itemsel.infoServ.uid, variacion.data)
                    .then((doc) => {
                  ...
                  this.servicioMod2
                    ...
                    .then(() => {
                      this.servicioMod2.updateVariciones(
                        this.itemsel.infoServ.uid,
                        variacion.id,
                        variacion.data
                      );
    ...
    if (this.validarVariacion(this.objectvariation)) {
      if (!this.editar) {
        ...
        this.variaciones.push({
          cfName: this.objectvariation.cfName,
          data: this.objectvariation,
          id: "nuevo",
        });
      }
      this.inicializarVariacion();
  ...
  validarVariacion(variation) {
    let bol = true;
    if (
      variation.cfName.trim() === "" ||
      variation.cfDescription.trim() === "" ||
      variation.cfPrice === 0
    ) {
      bol = false;
    }
  ...
  cambiarEstadoVariacion(pos, active) {
    ...
    if (!encontrado) {
      this.variacionesCambiadas.push({
        id: this.variaciones[pos].id,
        active: !active,
      });
    } else {
  ...
  agregarEquipo() {
    let bol = false;
    this.selection.selected.forEach((element) => {
      const do2 = this.itemsel.infoServ.equipos.find(
        (o) => o.id === element.id
      );
      if (!do2) {
