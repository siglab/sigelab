...
export class Modulo2Service {
  ...
  getEspaceForBuildAndPlace(edificio: string, espacio: string) {
    ...
    const refer = col.ref
      .where("spaceData.building", "==", edificio)
  ...
  getCollectionSolicitudesMantenimiento(labid) {
    ...
    const refer = col.ref
      .where("requestType", "==", "mantenimiento")
      .where("cfFacil", "==", labid);
    return refer.get();
  }
  ...
  getComponentForId(idequip, idcomp) {
    return this.afs
      .collection("cfEquip/" + idequip + "/components")
      .doc(idcomp)
      .ref.get();
  }
  ...
  updateCacheLaboratorios(uid, lab) {
    ...
    if (lab.hasOwnProperty("active")) {
      laboratorio["active"] = lab.active;
    }
    const data = {};
  ...
  setProgramacion(idprac, idprog, doc) {
    return this.afs
      .doc("practice/" + idprac + "/programmingData/" + idprog)
      .set(doc, { merge: true });
  }
  ...
  updateVariciones(idsrv, idVar, doc) {
    return this.afs
      .collection("cfSrv/" + idsrv + "/variations")
      .doc(idVar)
      .update(doc);
  }
  ...
  addAddrLaboratorio(idlab, doc) {
    return this.afs
      ...
      .add({
        ...
        cfClassScheme: "1227a225-db7a-444d-a74b-3dd4b438b420",
        cfEAddrValue: doc,
      });
  }
