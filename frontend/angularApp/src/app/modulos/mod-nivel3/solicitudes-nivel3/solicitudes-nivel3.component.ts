...
export class SolicitudesNivel3Component implements OnInit {
  ...
  constructor(
    ...
    private http: Http,
    private servicioSabs: SabsService
  ) {}
    ...
    if (this.moduloNivel3) {
      this.serviceMod3.getCollectionSolicitudes().then((data1) => {
        if (data1.size !== 0) {
          ...
          this.alertaError(
            "No se han generado solicitudes de mantenimiento aún"
          );
        }
      });
    }
      ...
      this.getFaculty().then((retor) => {
        ...
        for (const key in this.faculty) {
          if (this.faculty.hasOwnProperty(key)) {
            this.serviceMod3
              .getCollectionSolicitudesFacultad(key)
              .then((data1) => {
                data1.forEach((doc) => {
                  array.push(doc);
                });
  ...
  cambiarEstadoSolicitud(estado) {
    ...
    if (estado === "rechazado") {
      ...
      this.serviceMod3
        .updateSolicitudMantenimiento(this.solsel.uidsol, obj)
        .then(() => {
  ...
  estructurarSolicitudesActivas(data) {
    return new Promise((resolve, reject) => {
      ...
      data.forEach((element) => {
        ...
        console.log(899, elemento, elemento.cfFacil);
        this.serviceMod3.getLaboratorio(elemento.cfFacil).then((lab) => {
          if (lab.data() && lab.data() !== undefined) {
            this.serviceMod3
              .consultarNotificaciones(elemento.createdBy)
              .then((email) => {
                const Solicitud = {
                  uidsol: element.id,
                  ...
                  componentes: this.estructurarComponenteId(
                    elemento.relatedEquipments,
                    elemento.relatedComponents
                  ),
                ...
                if (elemento.relatedEquipments !== "") {
                  this.serviceMod3
                    .getEquipo(elemento.relatedEquipments)
                    .then((equipo) => {
  ...
  cambiarDataSolicitud(item) {
    ...
    if (item.idEquipo) {
      this.serviceMod3.getEquipo(item.idEquipo).then((data) => {
        this.consultarSabs(data.data().inventory)
          .then(() => {
            console.log("hecho");
  ...
  buscarCoincidenciasSolicitudes(item) {
    ...
    let montosol = 0;
    const fechaActual =
      parseInt(new Date().toISOString().split("-")[0], 10) - 2;
